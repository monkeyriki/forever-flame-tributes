  ...
  const { data: dbMemorials = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("memorials")
        .select("*")
        .eq("type", memorialType)
        .eq("visibility", "public")
        .eq("is_draft", false);
      if (error) throw error;
      return (data || []).map((m: any): Memorial => ({
        id: m.id,
        type: m.type as "human" | "pet",
        firstName: m.first_name,
        lastName: m.last_name || "",
        birthDate: m.birth_date || "",
        deathDate: m.death_date || "",
        location: m.location || "",
        photoUrl: m.image_url || "/placeholder.svg",
        bio: m.bio || "",
        tags: m.tags || [],
              ...
              <div className="mt-3 space-y-3 rounded-lg border border-border bg-card p-4 shadow-soft">
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    placeholder="Filtra per luogo (es. Roma)..."
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    placeholder="Anno di nascita (es. 1950)..."
                    className="flex-1 bg-transparent text-sm text-foreground placeholder:text-muted-foreground focus:outline-none"
                  />
                  <input
                    type="text"
                    value={yearDeathFilter}
                    onChange={(e) => setYearDeathFilter(e.target.value)}
                    placeholder="Anno di morte (es. 2024)..."
                    className="flex-1 bg-transparent text-sm text-foreground placeholder:text-muted-foreground focus:outline-none"
                  />
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    placeholder="Filtra per tag (es. Veterano, Golden Retriever)..."
                    className="flex-1 bg-transparent text-sm text-foreground placeholder:text-muted-foreground focus:outline-none"
                  />
                </div>
              </div>
