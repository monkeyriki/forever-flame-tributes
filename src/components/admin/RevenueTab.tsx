...
const chartConfig = {
  tributes: { label: "Tributi", color: "hsl(38, 45%, 55%)" },
  plans: { label: "Piani", color: "hsl(35, 30%, 35%)" },
};
...
const RevenueTab = () => {
  ...
  // Paid tributes as revenue source
  ...
  // Calculate monthly revenue from transactions + paid tributes
  const monthlyData = (() => {
    ...
    // Add paid tributes estimates (tier pricing)
  ...
  const pieData = [
    { name: "Tributi Virtuali", value: totalTributes || 1 },
    { name: "Piani Premium", value: totalPlans || 0 },
  ].filter((d) => d.value > 0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-foreground">€{totalTributes.toFixed(2)}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-foreground">€{totalPlans.toFixed(2)}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg font-sans">Ricavi Mensili (Ultimi 6 mesi)</CardTitle>
        </CardHeader>
        <CardContent>
          <ChartContainer config={chartConfig} className="h-[300px] w-full">
            <BarChart data={monthlyData}>
              ...
              <Bar dataKey="tributes" name="Tributi" fill="hsl(38, 45%, 55%)" radius={[4, 4, 0, 0]} stackId="a" />
              <Bar dataKey="plans" name="Piani" fill="hsl(35, 30%, 35%)" radius={[4, 4, 0, 0]} stackId="a" />
            </BarChart>
          </ChartContainer>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg font-sans">Distribuzione Ricavi</CardTitle>
        </CardHeader>
